...
export default function RootLayout({
  ...
  return (
    <html lang="en">
      ...
      <nav className="sticky top-0 group flex flex-col overflow-y-b-clip justify-between items-center px-2 hover:items-start md:px-4 py-6 gap-4 transition-all">
        <Image priority src={gradient} alt="" width={1080} height={1080} className="absolute h-full left-32 blur-3xl -z-1"/>
        <SideNav />
      </nav>
      <div className="flex flex-col flex-grow px-16 py-16 overflow-hidden z-10 rounded-l-[32px] bg-bodybg border border-label/[.08] md:overflow-y-auto md:p-12">{children}</div>
      </main>
